...
__global__ void create_texture_image_cuda_kernel(
    ...
    // if (fn>= 23144) {
    //     printf("out textures,fn: %d, i: %ld", fn, i);
    // }
    const scalar_t* vertices = &vertices_all[fn * 3 * 2];
    // if (fn>= 23144) {
    //     printf("out vertices,fn: %d, i: %ld", fn, i);
    // }
    ...
    /* compute face_inv 计算逆矩阵根据2D坐标点(根据的是这一步之前的image图像)*/
    ...
    /* compute w = face_inv * p 计算x，y点在三角形中的位置权重*/
    scalar_t weight[3];
...
at::Tensor create_texture_image_cuda(
    ...
    const long int image_size = image.numel();
    const dim3 blocks ((image_size / 3 - 1) / threads + 1, 1, 1);
    //打印所有参数
    printf("num_faces: %ld\n", num_faces);
    printf("texture_size_in: %ld\n", texture_size_in);
    printf("tile_width: %d\n", tile_width);
