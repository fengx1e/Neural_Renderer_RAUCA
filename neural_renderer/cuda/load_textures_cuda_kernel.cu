...
__global__ void load_textures_cuda_kernel(
  ...
  const int fn = i / (ts * ts * ts); //获得面索引，可能是face.shape(0)*textures_size*textures_size*textures_size的第i个
  ...
  scalar_t* face = &faces[fn * 3 * 2]; //确定face的起始位置，因为face是[face.shape(0),3,2]
  scalar_t* texture_ = &textures[i * 3]; //textures是[face.shape(0),textures_size,textures_size,textures_size,3]
