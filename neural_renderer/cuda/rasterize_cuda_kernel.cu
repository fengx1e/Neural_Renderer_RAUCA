...
__global__ void forward_face_index_map_cuda_kernel_1(
    ...
    const scalar_t* face = &faces[i * 9]; //face即为当前的面
    scalar_t* face_inv_g = &faces_inv[i * 9];

    /* return if backside */
    if ((face[7] - face[1]) * (face[3] - face[0]) < (face[4] - face[1]) * (face[6] - face[0])) //因为记录了正反面，当是反面的时候直接返回
        return;
...
__global__ void forward_face_index_map_cuda_kernel_2(
    ...
    for (int fn = 0; fn < nf; fn++) {
        ...
        //w则是对于三角形上的某一个点，根据它在三角形局部坐标系下的位置，计算出来的三个权重值。这三个权重值表示了该点相对于三角形三个顶点的权重，即可以用它们来插值计算该点在三角形上的属性值。
...
std::vector<at::Tensor> forward_face_index_map_cuda(
    ...
    //这个forward_face_index_map_cuda_1是找到每一个p[3][2]，即3个顶点在屏幕图片的坐标点位置，之后根据该坐标点，构建屏幕三角形的逆矩阵，放到faces中
